...
function dlx_solver() {
    ...
    for (i = 0; i < 9; i++) {
        for (j = 0; j < 9; j++) {
            var value = parseInt(lv_array[i][j]);
            if (value !== 0) {
...
function search() {
    ...
    while (d !== c) {
        ...
        //解析324列提供的信息中，哪些列被数应该填在哪些行，然后post当前output，再渲染
        // outgrid1 = new Array(9);
        // for (var i = 0; i < 9; ++i){
        //     outgrid1[i] = new Array(9);
        //     for (var j = 0; j < 9; ++j)
        //         outgrid1[i][j] = 0;
        // }
        // for (i = 0; i < size; ++i)
        ...
        //     outgrid1[parseInt(pos / 9)][pos % 9] = output[i] % 9 + 1;
        // }
        // postMessage()
